...
write_header() {
    echo -e "${CYAN}initai.dev - LLM Framework Manager ${GRAY}v${CURRENT_VERSION}${NC}"
...
save_configuration() {
    ...
    local llm="$3"
    local target_dir="$4"
    local launch_claude="$5"

    # Build JSON with optional launch_claude
    local json_content='{'
    json_content+='"base_url": "'$BASE_URL'",'
    json_content+='"framework": "'$framework'",'
    json_content+='"scope": "'$scope'",'
    json_content+='"llm": "'$llm'",'
    json_content+='"description": "'$framework' '$scope' development for '$llm'",'
    json_content+='"download_url": "/init/shared/'$framework'/'$scope$([ "$llm" != "universal" ] && echo "/$llm" || echo "")",'
    json_content+='"target_dir": "'$target_dir'",'
    json_content+='"last_updated": "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'",'
    json_content+='"script_version": "'$CURRENT_VERSION'"'

    # Add launch_claude if provided
    if [ -n "$launch_claude" ]; then
        json_content+=',"launch_claude": '$launch_claude
    fi
...
main() {
    ...
    if ! test_configuration; then
        ...
        save_configuration "$selected_framework" "$selected_scope" "$selected_llm" "$target_dir" "$launch_claude_preference"
