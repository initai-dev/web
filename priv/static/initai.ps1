...
function Write-Header {
    Write-Host "initai.dev - LLM Framework Manager " @Cyan -NoNewline
...
function Save-Configuration {
    param($framework, $scope, $llm, $targetDir, $launchClaude = $null)
    ...
    if ($launchClaude -ne $null) {
        $config.launch_claude = $launchClaude
    }
...
- Reference files: $targetDir (READ ONLY - do not modify or generate files here)

## Instructions
Read the framework guidelines from $targetDir at session start, but work in the main project directory:

- **Read** framework configuration from $targetDir (reference only)
...
- **Work in the main project directory** - NOT in the $targetDir folder

## File Structure
- CLAUDE.md - Initialization instructions (static - don't modify)
- PROJECT.md - Your working context (dynamic - update regularly)
- $targetDir/ - **READ-ONLY** framework reference files and templates
...
- Follow the framework's best practices from $targetDir reference
- Maintain consistency across the project
- **Save all project decisions and context to PROJECT.md**
- **IMPORTANT: Generate project files in main directory, NOT in $targetDir**
        ...
        try {
            ...
            if ($selectedLLM -eq "claude") {
                ...
                if ($launchClaude -match "^[yY]$") {
                    ...
                    $launchClaudePreference = $false
                }
            }

            Save-Configuration $selectedFramework $selectedScope $selectedLLM $targetDir $launchClaudePreference
